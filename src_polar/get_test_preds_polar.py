...
def main(cfg: DictConfig):
    ...
    x0_all = df_polar_partial[cfg.opt.features.partial.polar.feat1_aniso].values
    ...
    pred_df = df_polar_partial[['Molecule', 'Formula', 'Expt']].copy()
    
    pred_path = get_absolute_path(cfg.eval.all.polar.dir)
    ...
    for i, j in enumerate(list_of_x_all):
        ...
        predictions = stacked_nested_cv(new_list_of_models, list_of_models_names,list_of_spaces,
                                                  j, y_all, 10, 5, show_individual_test_pred = True)
